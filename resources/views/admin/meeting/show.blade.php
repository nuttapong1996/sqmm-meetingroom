<x-admin-layout title="{{ $meeting->title }}" titleVisible="hidden">
    <x-slot name="breadcrumbs">
        {{ Breadcrumbs::render('admin.meeting.show' ,$meeting->id) }}
    </x-slot>
                ...
                <div class="flex flex-col border border-gray-300 rounded-box p-6 mt-3 ">
                    ...
                    @if ($meeting->room_status_id == 2)
                        <i class="text-xs text-right text-gray-500 mt-2">ยกเลิกเมื่อ:
                            {{ $meeting->updated_at->thaidate('d/m/y H:i') }}น.<br>โดยคุณ {{ $meeting->cancel_user }}
                        </i>
                    @endif
                </div>
