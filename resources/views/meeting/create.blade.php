    ...
    <fieldset class="fieldset bg-base-100 border-base-300 rounded-box w-xs border p-4 lg:w-lg">
        ...
        <form action="{{ route('book.store') }}" method="POST" novalidate>
            ...
            <input name="meetingTitle" id="meetingTitle" type="text"
                class="input validator w-full @error('meetingTitle') input-error @enderror" placeholder="หัวข้อการประชุม"
                required pattern=".{3,}" value="{{ old('meetingTitle') }}"
            ...
            <input type="text" id="date-picker" name="end_date"
                class="input date-picker validator  w-full @error('end_date') input-error @enderror" placeholder="เลือกวันที่สิ้นสุด"
